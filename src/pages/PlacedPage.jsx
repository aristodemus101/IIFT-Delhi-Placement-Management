...
import { Download, RotateCcw, Search, Eye } from 'lucide-react'

export default function PlacedPage() {
  const { students, loading, unplace } = useStudents()
  ...
  const rows = filtered.map(s => [
    ...
    <span style={{
      ...
      background: 'var(--green-bg)', color: 'var(--green-text)', border: '1px solid var(--green-border)', fontWeight: 500
    }}>{s._placedCompany || '—'}</span>,
    <span style={{ fontSize: 12, color: 'var(--text-3)' }}>{s._placedAt ? new Date(s._placedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}</span>,
    <div style={{ display: 'flex', gap: 6 }}>
      <Btn size="sm" variant="ghost" onClick={() => setViewModal(s)}><Eye size={13} /></Btn>
      <Btn size="sm" variant="ghost" onClick={() => { if (confirm(`Move ${getVal(s, 'name')} back to active roster?`)) unplace(s._id) }} title="Move back to active">
